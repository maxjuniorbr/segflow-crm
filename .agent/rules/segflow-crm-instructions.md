---
description: Convencoes e padroes do projeto SegFlow CRM — lido automaticamente em toda interacao
alwaysApply: true
---
...
## Linguagem e tom
- Codigo e documentacao (como o README) em ingles; interface, textos e mensagens ao usuario em pt-BR.
...
## Modais e ConfirmDialog
- `useModalBehavior` em `src/shared/hooks/useModalBehavior.ts`: hook compartilhado para focus trap, Escape, body overflow lock e restauracao de foco. Usar em todo modal/dialog.
- `ConfirmDialog`: API `isOpen`, `onConfirm`, `onCancel`, `title`, `message`, `confirmText`, `cancelText`, `variant`. Nao usar props `onClose`/`type` (nao suportadas). Usa `useModalBehavior` internamente.
...
- Backend segue Clean Architecture:
  ...
  - Repositories isolam acesso ao banco; helpers compartilhados em `server/src/infrastructure/repositories/queryHelpers.js`.
...
- Catch blocks que nao usam o erro devem usar bare `catch {` (sem parametro), nao `catch (_err)`.
...
## Testes
- Vitest para backend e frontend; Testing Library + vitest-axe para componentes React.
- Estrutura backend: `tests/controllers/`, `tests/unit/`, `tests/functional/`, `tests/security/`.
- Mocks centralizados via convencao `__mocks__/` do Vitest (ex.: `server/config/__mocks__/db.js`, `__mocks__/bcryptjs.js`).
- Helpers compartilhados em `tests/utils/`: `controllerTestUtils.js`, `testFactories.js`, `integrationTestUtils.js`, `testDbMock.js`.
- Novas features e bugfixes devem incluir testes; rodar suite antes de declarar tarefa concluida.
- Nao afirmar que testes passam sem executar e confirmar a saida — evidencias antes de afirmacoes.
...
- Regex de validacao devem ser livres de ReDoS: nao usar lookaheads com `.*`; preferir testes individuais por classe de caractere.
- Logs de desenvolvimento devem sanitizar dados user-controlled contra log injection.

## Jules (agente AI externo)
- O projeto usa o [Jules](https://jules.google.com) como agente de codigo complementar.
- Se `.env.local` existir na raiz com `JULES_API_KEY`, consultar sessoes e sugestoes do Jules via API REST (`https://jules.googleapis.com/v1alpha`) para verificar apontamentos pendentes.
- Endpoints uteis: `GET /sessions` (listar tarefas), `GET /sessions/{id}` (detalhar com patch), `GET /sources` (repositorios conectados).
- Header de autenticacao: `x-goog-api-key: <JULES_API_KEY>`.
- Memorias/Knowledge nao sao acessiveis via API; apenas pelo painel web.
